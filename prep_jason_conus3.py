...
print('running prep_jason_conus3 for ',areg,ascl,skip_cold_season,flush=True)
...
for adate in dates:
    ...
    if skip_cold_season:
        if thedate.month < 4: continue
        if thedate.month > 9: continue
    ...
    if thedate.year in [2018,2019]:
        ...
        if not os.path.exists(c07file): continue
        if not os.path.exists(c09file): continue
        if not os.path.exists(c13file): continue
        if not os.path.exists(glmfile): continue
        if not os.path.exists(mrmsfile): continue
        ...
        sfolder = '/mnt/lhmlnas/conus3/yoonjin_conus3/'+thedate.strftime('%Y/%m%d/')
    ...
    c07 = c07[ regions[areg]['iy0']:regions[areg]['iy1'], regions[areg]['ix0']:regions[areg]['ix1'] ]
    c09 = c09[ regions[areg]['iy0']:regions[areg]['iy1'], regions[areg]['ix0']:regions[areg]['ix1'] ]
    c13 = c13[ regions[areg]['iy0']:regions[areg]['iy1'], regions[areg]['ix0']:regions[areg]['ix1'] ]
    glm = glm[ regions[areg]['iy0']:regions[areg]['iy1'], regions[areg]['ix0']:regions[areg]['ix1'] ]
    mrms = mrms[ regions[areg]['iy0']:regions[areg]['iy1'], regions[areg]['ix0']:regions[areg]['ix1'] ]

    bad = (c07 <= 0) | (c09 <= 0) | (c13 <= 0) | (glm < 0) | (mrms <= -999)

    c07 = (scalings[ascl]['c07']['max'] - c07) / (scalings[ascl]['c07']['max'] - scalings[ascl]['c07']['min'])
    c09 = (scalings[ascl]['c09']['max'] - c09) / (scalings[ascl]['c09']['max'] - scalings[ascl]['c09']['min'])
    c13 = (scalings[ascl]['c13']['max'] - c13) / (scalings[ascl]['c13']['max'] - scalings[ascl]['c13']['min'])
    glm = (glm - scalings[ascl]['glm']['min']) / (scalings[ascl]['glm']['max'] - scalings[ascl]['glm']['min'])
    mrms = (mrms - scalings[ascl]['mrms']['min']) / (scalings[ascl]['mrms']['max'] - scalings[ascl]['mrms']['min'])
    ...
    outfile = 'conus3/jason_conus3/'+afolder+'/conus3_reg'+areg+'_scl'+ascl+'_'+str(fcnt[afolder]).zfill(6)+'.npz'
    print('writing ',outfile,flush=True)

    np.savez(outfile, xdata=xdata, ydata=ydata)

    fout.write(afolder+' '+str(fcnt[afolder]).zfill(6)+' '+thedate.strftime('%Y-%m-%d_%H:%MZ')+'\n')
